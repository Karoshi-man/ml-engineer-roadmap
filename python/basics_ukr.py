...
# Type Casting (Приведення типів)
    # процес перетворення одного типу даних в інший
    # оскільки Python має строгу типізацію, він автоматично не додасть "5" (str) до 5 (int)
    # ви повинні конвертувати типи явно

x = 10      # int
y = "20"    # str
z = 10.5    # float

# int_val = x + y      # Помилка: unsupported operand type(s) for +

# Explicit Conversion (Явне перетворення)
int_val = int(y)       # "20" -> 20
float_val = float(x)   # 10 -> 10.0
str_val = str(z)       # 10.5 -> "10.5"
bool_val = bool(0)     # 0 -> False (Будь-яке не нульове значення — True)

print(x + int_val)     # 30 (Тепер це працює)



# Working with Strings (Робота з рядками)

# String Slicing [start:stop:step] (Зрізи рядків)
    # Витягує частину рядка
    # start: включно (за замовчуванням 0)
    # stop: виключно (не входить у діапазон, за замовчуванням довжина рядка)
    # step: крок (за замовчуванням 1)

text = "Python Programming"

print(text[0:6])       # "Python" (від індексу 0 до 5)
print(text[:6])        # "Python" (початок мається на увазі)
print(text[7:])        # "Programming" (від 7 до кінця)
print(text[::-1])      # "gnimmargorP nohtyP" (перевертає рядок)

# String Methods (Методи рядків)
    # Рядки незмінні (immutable), 
    # тому методи повертають НОВИЙ рядок

s = "  Hello World!  "

print(s.lower())       # "  hello world!  " (все маленькими)
print(s.upper())       # "  HELLO WORLD!  " (все великими)
print(s.strip())       # "Hello World!" (видаляє пробіли з країв)
print(s.replace("!", "?")) # "  Hello World?  " (замінює підрядки)

# Splitting and Joining (Розділення та об'єднання)
...
# Operations with Data Structures (Операції зі структурами даних)

# List Operations (Mutable/Змінні)
    # Ми можемо змінювати списки: додавати, видаляти або змінювати елементи

fruits = ["apple", "banana"]

    # Adding (Додавання)
fruits.append("cherry")      # Додає в кінець: ["apple", "banana", "cherry"]
fruits.insert(1, "orange")   # Додає на індекс 1: ["apple", "orange", "banana", "cherry"]

    # Removing (Видалення)
item = fruits.pop()          # Видаляє і повертає останній елемент ("cherry")
fruits.remove("apple")       # Видаляє перше входження значення

    # Accessing (Доступ)
first = fruits[0]            # "orange"


# Dictionary Operations (Mutable/Змінні)
    # Доступ та зміна пар ключ-значення

user = {"name": "Alex", "role": "admin"}

    # Accessing (Доступ)
# print(user["age"])         # KeyError: 'age' не існує
print(user.get("age"))       # None (Більш безпечний спосіб, без помилки)
print(user.get("age", 18))   # 18 (Значення за замовчуванням, якщо ключа немає)

    # Modifying (Зміна)
user["age"] = 25             # Додає нову пару або оновлює існуючу
user["name"] = "Alexander"   # Оновлює значення

    # Methods (Методи)
keys = user.keys()           # Повертає об'єкт-список ключів
values = user.values()       # Повертає об'єкт-список значень


# Set Operations (Mutable, Unique) / (Множини)
    # Корисні для математичних операцій та видалення дублікатів

a = {1, 2, 3}
b = {3, 4, 5}

a.add(6)                     # Додає елемент
a.remove(1)                  # Видаляє елемент

print(a.union(b))            # {2, 3, 4, 5, 6} (Об'єднання / АБО)
print(a.intersection(b))     # {3} (Перетин / І)
print(a.difference(b))       # {2, 6} (Різниця / a - b)



# Functions (Deep Dive) / (Функції детально)
    # Блоки коду для повторного використання
    # Ключові концепції: Аргументи, Return, Scope (Область видимості)

# Function with Default Arguments (Аргументи за замовчуванням)
def greet(name, msg="Good morning"):
    """
    Якщо 'msg' не передано, використовується значення за замовчуванням
    """
    return f"{msg}, {name}!"
...
def unlimited_sum(*args):
    # args стає кортежем (tuple) усіх позиційних аргументів
    total = 0
    for num in args:
        total += num
    return total
...
# Scope (Local vs Global) / (Область видимості)
global_var = 100

def my_func():
    local_var = 10
    # print(global_var) # Працює (Доступ на читання)
    # global_var = 50   # Помилка! Щоб змінити, треба написати 'global global_var'

# Lambda Functions (Лямбда-функції)
    # Маленькі анонімні функції. Синтаксис: lambda arguments : expression
    # Часто використовуються всередині інших функцій, як map() або filter()
...
# Exceptions (Error Handling) / (Обробка помилок)
    # Керування помилками під час виконання, щоб програма не "падала"

    # try: блок коду, який ми пробуємо виконати
    # except: блок, який спрацьовує, якщо виникла помилка
    # else: спрацьовує, якщо помилки НЕ було
    # finally: спрацьовує ЗАВЖДИ (для очищення ресурсів)
...
# Built-in Functions Overview (Огляд вбудованих функцій)
    # Python має багато готових функцій
    # Ось найкорисніші, які ще не згадувалися:

abs(-5)           # 5 (Модуль числа)
max(1, 2, 3)      # 3 (Максимум)
min(1, 2, 3)      # 1 (Мінімум)
sum([1, 2, 3])    # 6 (Сума)
len("hello")      # 5 (Довжина послідовності)
type(x)           # Повертає тип об'єкта
id(x)             # Повертає унікальну адресу в пам'яті
sorted([3, 1, 2]) # Повертає новий відсортований список [1, 2, 3]
help(print)       # Друкує документацію



# Escape Characters & Sequences (Екранування символів)
    # Зворотна коса риска (\) — це символ екранування.
    # Вона дозволяє використовувати спецсимволи, які інакше неможливо ввести.

print("Hello\nWorld")    # \n = Новий рядок (New Line)
print("Name:\tJohn")     # \t = Табуляція (Tab)
print('It\'s me')        # \' = Одинарна лапка (всередині одинарних лапок)
print("Backslash: \\")   # \\ = Щоб надрукувати саму зворотну косу риску


# Multiline Statements (Багаторядкові інструкції)
    # Python зазвичай очікує одну команду на одному рядку.
    # Але довгі рядки можна розбивати для читабельності.

    # 1. Explicit Line Continuation (Явне продовження через \)
total = 10 + \
        20 + \
        30

    # 2. Implicit Line Continuation (Неявне продовження - Рекомендовано)
    # Вирази всередині дужок (), [], або {} можна розбивати без слешів.
total_sum = (
    10 + 
    20 + 
    30
...
# Multiple Assignment (Множинне присвоєння)
    # Ви можете присвоїти значення декільком змінним в один рядок.

    # Unpacking (Розпакування)
x, y, z = 1, 2, "Hello" 
# x=1, y=2, z="Hello"

    # Chained Assignment (Ланцюгове присвоєння)
a = b = c = 100 
# Всі три змінні вказують на один і той самий об'єкт (100)


# Deleting Variables (Видалення змінних)
    # Ключове слово 'del' видаляє посилання на об'єкт.
    # Воно стирає ім'я змінної з поточної області видимості.
...
# Constants (Константи)
    # У Python НЕМАЄ строгого типу 'const' (як у C++ або Java).
    # Ми використовуємо Домовленість про іменування.
    # КОНСТАНТИ_ПИШУТЬСЯ_ВЕЛИКИМИ_ЛІТЕРАМИ.

PI = 3.14159
MAX_CONNECTIONS = 1000

# Технічно, ви все одно можете їх змінити (PI = 3.0),
# але інші програмісти знатимуть, що цього робити не варто.


# Signed vs Unsigned Integers (Зі знаком та Без знаку)
    # У мовах типу C/C++ є 'int' (зі знаком) та 'unsigned int' (тільки позитивні).
    
    # У Python УСІ цілі числа (integers) мають знак (signed objects).
    # Числа в Python мають довільну точність (можуть бути наскільки великими, скільки дозволяє RAM).
    # Вам не треба хвилюватися про переповнення (overflow) чи типи без знаку.

positive_num = 10
negative_num = -10

    # Якщо вам треба працювати з конкретним бінарним представленням (наприклад, unsigned 8-bit),
    # зазвичай використовують побітові операції або бібліотеки 'ctypes' чи 'struct'.
    
x = -5
unsigned_val = x & 0xFF  # Імітація unsigned byte (251)


# Command Line Arguments (Аргументи командного рядка)
    # Як обробити аргументи, передані скрипту під час запуску з терміналу.
    # Приклад команди: python script.py arg1 arg2
    # Ми використовуємо модуль 'sys'.
